...
public class TestPaper extends TestHibernate {
    ...
    public void create() {
        ...
        paper.setTitle(name);
        Set<Collaboration> allCollaborations = new HashSet<Collaboration>(
            Arrays.asList(Collaboration.WITH_PDF,
                Collaboration.WITH_STUDENT,
    ...
    public void getPapersTitleMatching() {
        ...
        Set<Paper> papers = paperDAO.getPapersTitleMatching(getMethodName(), 0,
            10);
    ...
    public void saveDuplicateDOI() {
        ...
        try {
            paperDAO.save(paper);
            Assert.fail("no exception when saving new paper with existing DOI");
        } catch (DataIntegrityViolationException e) {
            log.error(e.getMessage());
            Assert.assertTrue(true);
        }
    ...
    public void paperAuthors() {
        ...
        Set<AuthorRanked> authorsRanked = new HashSet<AuthorRanked>();
        int numAuthors = 10;
        for (int i = 0; i < numAuthors; ++i) {
            ...
            AuthorRanked authorRanked = new AuthorRanked();
            authorRanked.setPaper(paper);
            authorRanked.setAuthor(author);
            authorRanked.setRank(i);
            getJdbcUtils().addAuthorRanked(authorRanked);
            authorsRanked.add(authorRanked);
        }
        Assert.assertEquals(numAuthors, countRowsInTable("author"));

        paper.getAuthors().addAll(authorsRanked);
        paper = paperDAO.save(paper);
        paperDAO.flush();
        ...
        for (int i = 0; i < numAuthors; ++i) {
            authorRanked = paper.getAuthors().iterator().next();

            Assert.assertEquals(i, authorRanked.getRank());

            authorsRanked.remove(authorRanked);
            paper.getAuthors().remove(authorRanked);
            paper = paperDAO.save(paper);
            paperDAO.flush();

            Assert
                .assertEquals(authorsRanked.size(), paper.getAuthors().size());
        }
    }
    ...
    public void relatedPapers() {
        ...
        Assert.assertEquals(relatedPaper.getId(), papers.iterator().next()
            .getId());
    }
}
