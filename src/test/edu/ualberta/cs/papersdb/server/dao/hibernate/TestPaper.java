...
public class TestPaper extends TestHibernate {
	...
	public void saveDuplicateTitle() {
		...
		try {
			paperDAO.save(paper);
			Assert.fail("no exception when saving new paper with existing title");
	...
	public void paperPublication() {
		...
		Assert.assertNotNull("autogen key shouldn't be null", publisher.getId());
