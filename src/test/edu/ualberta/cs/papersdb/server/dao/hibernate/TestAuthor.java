...
public class TestAuthor extends TestHibernate {
    ...
    public void getByFamilyNames() {
        ...
        try {
            result =
                authorDAO.getByFamilyName(new BigInteger(130, getR())
                    .toString());
            Assert
                .fail("no exception when getting author with family names not in db");
    ...
    public void getMatchingFamilyName() {
        ...
        Set<Author> authors =
            authorDAO.getMatchingFamilyName(getMethodName(), 0, 10);
